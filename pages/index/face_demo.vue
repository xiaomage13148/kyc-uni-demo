<template>
  <view class="content">
    ...
    <view class="btn-row">
      <button type="primary" class="primary" @tap="startLiveCheckAndCompareWithIdImgService">身份证对比
      </button>
      <button type="primary" class="primary" @tap="startLiveCheckAndCompareWithScrImgService">
        自带对比源
      </button>
      <button type="primary" class="primary" @tap="startLiveCheckService">仅活体检测</button>
      <!--      <button type="primary" class="primary" @tap="startAdvanceLiveCheckAndCompareWithIdImgService">-->
      <!--        增強身份证对比-->
      <!--      </button>-->
      <!--      <button type="primary" class="primary"-->
      <!--              @tap="startAdvanceLiveCheckAndCompareWithScrImgService">增強自带对比源-->
      <!--      </button>-->
      <!--      <button type="primary" class="primary" @tap="startAdvanceLiveCheckService">增強仅活体检测</button>-->
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    // compareType：0:身份证对比
    startLiveCheckAndCompareWithIdImgService() {
      ...
      this.preStartWbFaceVerifyService(compareTypeConstant.idCardComparison, advanceConstant.basic);
    },

    // compareType：1:自带对比源
    startLiveCheckAndCompareWithScrImgService() {
    ...
    preStartWbFaceVerifyService(compareType, advance) {
      ...
      if (advance === advanceConstant.basic) {
        ...
        // TODO 增强版待完善
        console.log('---->打印增强版待完善' , advance);
      }

    },

    // TODO暂时去除无用代码
/*    getFaceIdSign(compareType, advance) {
      console.log('【uni log】METHOD ================> getSign:compareType=' + compareType + '.');
      uni.request({
        method: 'GET',
        url: 'https://miniprogram-kyc.tencentcloudapi.com/ems-partner/cert/sign', //仅为示例，并非真实接口地址。
        data: {
          appid: this.appid,
          nonce: this.nonce,
          userid: this.userId,
        ...
        success: (res) => {
          ...
          console.log('【uni log】getSignSuccess sign=' + sign);
          if (sign !== '') {
            this.getFaceId(sign, compareType, advance);
          }
        },

      });
    },
    getFaceId(sign, compareType, advance) {
      console.log('【uni log】METHOD ================> getFaceId:sign=' + sign
          + ',compareType=' + compareType);

      console.log('【uni log】this.usernameInput=' + this.usernameInput +
          ', this.useridInput=' + this.useridInput);

      // 请求的参数
      let srcPhotoSrc = '';
      let srcPhotoType = '';
      //如果是自带对比源，需要送入对比源类型和对比照片
      if (compareType === 1) {
        srcPhotoSrc = '此处需要塞入对比源图片的base64 string';
        srcPhotoType = '此处需要塞入对比类型'; //1是水纹照  2是高清照
      ...
      if (advance === 0) {
        uni.request({
          ...
          data: {
            ...
            idNo: this.useridInput,
            userId: this.userId,
            srcPhotoSrc: srcPhotoSrc,
            srcPhotoType: srcPhotoType,
          ...
          success: (res) => {
            ...
            if (faceId) {
              console.log('startFaceVerify: faceid=' + faceId);
              this.getServiceSign(compareType, faceId, advance);
            } else {
              uni.showToast({
                icon: 'none',
        ...
        if (compareType === 0) {
          uni.request({
            method: 'POST',
            url: 'https://miniprogram-kyc.tencentcloudapi.com/api/server/getAdvFaceId',
            data: {
              webankAppId: this.appid,
              orderNo: this.orderNo,
              name: this.usernameInput,
    ...
    getServiceSign(compareType, faceId, advance) {
      ...
      uni.request({
        method: 'GET',
        url: 'https://miniprogram-kyc.tencentcloudapi.com/ems-partner/cert/signature', //仅为示例，并非真实接口地址。
        data: {
          appid: this.appid,
          nonce: this.nonce,
          userid: this.userId,
        ...
        face.startWbFaceVerifyService({
          ...
          if (scene === 'wb_face_callback_verify_result') {
            ...
            if (success) {
              ...
              if (domain === 'WBFaceErrorDomainCompareServer') {
                console.log(
                    '对比失败，liveRate=' + res.liveRate + '; similarity=' + res.similarity + ';sign=' +
                    res.sign);
                uni.showToast({
                  icon: 'none',
                  title: '对比失败，liveRate=' + res.liveRate + '; similarity=' + res.similarity +
